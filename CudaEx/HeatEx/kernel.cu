...
struct DataBlock {
    ...
    float           totalTime;
    float           frames;
    DataBlock() :totalTime(0), frames(0) {}
};
...
__global__ void blend_kernel(float* outSrc, const float* inSrc)
{
    ...
    outSrc[offset] = inSrc[offset] + SPEED * (
        (inSrc[top] + inSrc[bottom] + inSrc[left] + inSrc[right]) //sum
        - inSrc[offset] * 4);
...
void anim_gpu(DataBlock* d, int ticks) {
    ...
    ++d->frames;
    printf("Average Time per frame:  %3.1f ms,frame num %f\n",
        d->totalTime / d->frames, d->frames);
}
...
int main()
{
    ...
    for (int i = 0; i < DIM * DIM; i++)
    {
        ...
        int x = i % DIM;
        int y = i / DIM;
        if ((x > 500) && (x < 600) && (y > 310) && (y < 410))
            temp[i] = MAX_TEMP;
    }
    temp[DIM * 100 + 100] = (MAX_TEMP + MIN_TEMP) / 2;
    temp[DIM * 400 + 150] = MIN_TEMP;
    temp[DIM * 200 + 200] = MIN_TEMP;
    temp[DIM * 200 + 400] = MIN_TEMP;
    ...
    HANDLE_ERROR(cudaMemcpy(data.dev_constSrc,
        temp, bitmap.image_size(), cudaMemcpyHostToDevice));

    for (int y = 600; y < DIM - 200; y++)
    {
